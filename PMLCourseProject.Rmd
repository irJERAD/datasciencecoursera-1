...
With some prior test with the pure randomForest() function from the randomForest library to find out how quickly a single training can be done, the following model including some cross-validation is carried out:
...
outOfSampleError
```
The accuracy on the validation set was pretty equal to that of the test set, wich is is a good sign that the model will deal well with unknown data and was not overfitted to the training data set. The predicted out of sample error is `r outOfSampleError`. 
